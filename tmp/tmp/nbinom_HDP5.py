...
class NBinomModel(object):
    ...
    def save(self, outdir):
        ...
        ## save chains
        pars = np.hstack([self.iter, self.nact, self.mu1, self.tau1, np.mean(self.log_mu), np.var(self.log_mu),
                          self.m0, self.t0, self.eta])
    ...
    def _update_hpars(self):
        ...
        # self.tau2 = st.sample_normal_prec_jeffreys(s1, s2, n)
...
def _update_group_vars(args):
    ...
    # ## update d: 2
    # iact = np.bincount(d, minlength=lu.size) > 0
    # k_ = rn.choice(np.nonzero(iact)[0])
    k_ = 0
    d_ = np.tile(k_, nfeatures)

    loglik2 = _compute_loglik(([counts_norm], nreplicas), log_phi, log_mu, beta.reshape(-1, 1)[c[d]]).sum(-1)
    loglik2_ = _compute_loglik(([counts_norm], nreplicas), log_phi, log_mu, beta.reshape(-1, 1)[c[d_]]).sum(-1)

    idxs = (loglik2_ >= loglik2) | (rn.rand(*loglik2.shape) < np.exp(loglik2_ - loglik2))
    d[idxs] = d_[idxs]
